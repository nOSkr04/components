...
const CreatePlaceSheet = React.memo(() => {
  ...
  const date1= new Date("2023-01-18");
    ...
    <SafeAreaView style={styles.container}>
      ...
      <Modal isVisible={isModalVisible}>
        <View style={styles.modalContainer}>
          <Text onPress={() => {setInfinity(!infinity);}} style={styles.text} >{infinity ? "infinity" : "limited"}</Text>
          <Text style={styles.text} >{differenceInDays(date,date1)}</Text>
          {
            !infinity && 
              <DateTimePicker
